...
case class OrdersService(ordersActor: ActorRef) extends ApiRoute {
  ...
  @ApiResponses(Array(
    new ApiResponse(code = 200, message = "Return Order Id", response = classOf[OrderCreated]),
  ...
  @ApiResponses(Array(
    new ApiResponse(code = 200, message = "Return sum", response = classOf[OrderCreated]),
    new ApiResponse(code = 500, message = "Internal server error")
  ))
  def add1 =
    path("add1") {
